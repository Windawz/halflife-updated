...
#include "windawz_cvars.h" // for Windawz::cl_viewrollangle
...
void V_CalcViewRoll(struct ref_params_s* pparams)
{
	...
	// Windawz: use my CVars instead:
	float viewrollangle = Windawz::cl_viewrollangle->value;
	// Reverse, so that higher speed means _faster_ rolling, plus zero check
	// Multiply to allow sane input values
	float viewrollspeed = ((Windawz::cl_viewrollspeed != 0) ? 1 / Windawz::cl_viewrollspeed->value : 0) * 10000;
	side = V_CalcRoll(viewentity->angles, pparams->simvel, viewrollangle, viewrollspeed);
